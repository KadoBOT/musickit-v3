...
declare namespace MusicKit {
  ...
  type APIEndpoints = {
    "/v1/catalog/{{storefrontId}}/albums": {
      params: { ids: string[] };
      response: ReturnType<API["albums"]>;
    };
    "/v1/catalog/{{storefrontId}}/search/suggestions": {
      params: { kinds: SearchKind[]; term: string; types: ["songs"] };
      response: Data<MusicKitAPI.SearchSuggestionsResponse>;
    };
    "/v1/catalog/{{storefrontId}}/search/hints": {
      params: { term: string } & QueryParameters;
      response: Data<MusicKitAPI.SearchHintsResponse>;
    };
    "/v1/catalog/{{storefrontId}}/search": {
      ...
      response: Data<MusicKitAPI.SearchResponse>;
    };
    "/v1/catalog/{{storefrontId}}/songs": {
      params: { "filter[equivalents]": string };
      response: Data<MusicKitAPI.SongsResponse>;
    };
    "/v1/language/{{storefrontId}}/tag": {
      params: { acceptLanguage: string };
      response: Data<MusicKitAPI.LangageTagResponse>;
    };
  };
  type SearchType =
    | "songs"
  ...
  type DynamicEndpoints<T> = T extends keyof APIEndpoints
    ? APIEndpoints[T]
    : T extends `${infer L}?${string}`
    ? L extends keyof APIEndpoints
      ? APIEndpoints[L]
      : never
    : never;
   ...
   */
  interface API {
    music: <T extends keyof APIEndpoints | (string & {})>(
      endpoint: T,
      // params: T extends `${infer L}?${string}`
      //   ? L extends keyof APIEndpoints
      //     ? APIEndpoints[L]["params"]
      //     : never
      //   : APIEndpoints[T]["params"],
      params: DynamicEndpoints<T>["params"],
      options?: RequestInit
    ) => Promise<DynamicEndpoints<T>["response"]>;
    /**
