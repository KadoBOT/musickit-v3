...
declare namespace MusicKit {
	...
	type SearchKind = "terms" | "topResults"
	type Data<T> = { data: T }
	interface APIEndpoints {
		'/v1/catalog/{{storefrontId}}/albums': { params: { ids: string[] }, response: ReturnType<API['albums']> };
		'/v1/catalog/{{storefrontId}}/search/suggestions': { params: { kinds: SearchKind[], term: string, types: ["songs"] }, response: Data<MusicKitAPI.SearchSuggestionsResponse> };
		'/v1/catalog/{{storefrontId}}/search/hints': { params: { term: string }, response: Data<MusicKitAPI.SearchHintsResponse> };
		'/v1/catalog/{{storefrontId}}/search': { params: { term: string, types: ("activities" | "albums" | "apple-curators" | "artists" | "curators" | "music-videos" | "playlists" | "record-labels" | "songs" | "stations")[] }, response: Data<MusicKitAPI.SearchResponse> };
		'/v1/catalog/{{storefrontId}}/songs': { params: { 'filter[equivalents]': string }, response: Data<MusicKitAPI.SongsResponse>}
	}
	 ...
	 */
	interface API {
		music: <T extends keyof APIEndpoints>(endpoint: T, params: APIEndpoints[T]['params'], options?: RequestInit) => Promise<APIEndpoints[T]['response']>
		/**
